...
class clf_Arc(nn.Module):
    def __init__(self, args, out_dim):
        super(clf_Arc, self).__init__()
        self.easy_margin = True
        self.criterion = torch.nn.CrossEntropyLoss() 
        self.weight = Parameter(torch.Tensor(out_dim, args.out_dim)) 
...
class clf_Cos(nn.Module):
    def __init__(self, args, out_dim):
        super(clf_Cos, self).__init__()
        self.criterion = torch.nn.CrossEntropyLoss() 
        self.weight = Parameter(torch.Tensor(out_dim, args.out_dim)) 
        nn.init.xavier_uniform_(self.weight)   
        self.s = args.s
...
class clf_Classic(nn.Module):
    def __init__(self, in_dim = 1600, out_dim=5):
        super(clf_Classic, self).__init__()        
...
#        super(clf_Cos, self).__init__()
#        self.L = nn.Linear(args.out_dim, outdim, bias = False)
#        
#        self.class_wise_learnable_norm = True  #See the issue#4&8 in the github 
#        if self.class_wise_learnable_norm:      
#            WeightNorm.apply(self.L, 'weight', dim=0) #split the weight update component to direction and norm      
#
#        self.scale_factor = 5; #2a fixed scale factor to scale the output of cos value into a reasonably large input for softmax, for to reproduce the result of CUB with ResNet10, use 4. see the issue#31 in the github 
#
#         
#         
#        self.criterion = torch.nn.CrossEntropyLoss()
...
#    def loss(self, cons_theta_m, y):
#        y_1Hot = torch.zeros_like(cons_theta_m)         ### [25, 5]
#        y_1Hot.scatter_(1, y.view(-1, 1), 1)            ### [25, 5]
#        logits = (y_1Hot * cons_theta_m) + ((1.0 - y_1Hot) * self.cosine_theta)
#        
#        logits = self.s * logits
#        loss = self.criterion(logits, y)
#        return loss
