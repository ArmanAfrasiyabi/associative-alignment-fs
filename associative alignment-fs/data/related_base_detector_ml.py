...
import shutil
from backbone.utils import clear_temp
...
def saveLoad_base_embedding(args, net, fs_approach):
...
def related_base_data(args, net, z_embed, z_support, folders, device, n_B):
    clf_simMat = torch.zeros(len(z_embed), args.test_n_way)
    for b in range(len(z_embed)):
        zb = z_embed[b].to(device) 
        logits = net.score_loss(args, z_support, z_query=zb, score = True, rb = True)
